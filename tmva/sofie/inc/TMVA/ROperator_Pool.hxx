...
public:
   enum OpMode { Invalid, AveragePool, MaxPool };
   ...
   ROperator_Pool(PoolType type, RAttributes_Pool attr, std::string nameX, std::string nameY)
      : fPoolType(type), fAttrCeilMode(attr.ceil_mode), fAttrCountIncludePad(attr.count_include_pad),
        fAttrStorageOrder(attr.storage_order), fAttrAutopad(attr.auto_pad),
        fAttrDilations(dilations), fAttrKernelShape(kernelShape), fAttrPads(pads), fAttrStrides(strides),
   ...
   std::string Generate(std::string OpName) {
      ...
      // Perform Pool operation- Maxpool or average pool depending on mode type 
