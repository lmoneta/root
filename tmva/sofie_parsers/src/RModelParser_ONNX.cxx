...
std::unique_ptr<ROperator> make_ROperator_Pool(const onnx::NodeProto& nodeproto, const onnx::GraphProto& /* graphproto */, std::unordered_map<std::string, ETensorType>& tensor_type) {
   ...
   ROperator_Pool::OpMode op_mode = ROperator_Pool::Invalid;
   if (nodeproto.op_type() == "MaxPool")
      op_mode = ROperator_Pool::MaxPool;
   else if (nodeproto.op_type() == "AveragePool")
      op_mode = ROperator_Pool::AveragePool;
   ...
   for (int_t i = 0; i < nodeproto.attribute_size(); i++) {
      ...
      if (attribute_name == "auto_pad") {
         ...
         attr.ceil_model = nodeproto.attribute(i).i();
      } else if (attribute_name == "count_include_pad" && op_mode == ROperator_Pool::AveragePool) {
         attr.count_include_pad = nodeproto.attribute(i).i();
      } else if (attribute_name == "storage_order" && op_mode == ROperator_Pool::MaxPool) {
         attr.storage_order = nodeproto.attribute(i).i();
      } else if (attribute_name == "dilations" && op_mode == ROperator_Pool::MaxPool) {
